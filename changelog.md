...
- Certain materials now appear in chest loot:
  - Cognitive Flux may be found in ruined portal chests
  - Astute Assemblies may be found in desert temple chests
  - Forgotten Dust may be found in stronghold library chests and abandoned mineshafts
- Increased Cognitive toolset durability from 835 -> 921
...
[Changes]
- Forgotten dust drops from infected bookshelves are now affected by fortune
...
- Increased the number of orbs the Amulets can collect per batch
- Increased the number of orbs the Obelisk can collect per batch
- Increased Fortuitous Amulet XP bonus
...
- Decreased Linear Accelerator area of effect by 0.1 on each side to make it easier to walk between
activated accelerators without being launched
...
- Big rework to Memory Tablet:
  - Memory Tablet can now be used to remotely access an Obelisk up to 48 blocks away
  - However, as a consequence of this change, there is now a one-to-one player-to-obelisk correspondence
  - This reverts the change in v2.4.2 -- Players can no longer link themselves to multiple Obelisks
  - Obelisks still retain player data if they are moved around
  - There is no change to experience recovery functionality
- Changed Memory Tablet recipe to require an enchanted book (any)
- Added config option to allow Enlightened Amulet to ignore orbs spawned by bookshelves (false by default)
...
[Changes]
- Reworked Fortuitous Amulet xp mechanics to give a diminishing boost to greater xp drops, rather than capping the effect off at 20 XP
- Obelisks linked using a Memory Tablet now save player data. This means Obelisks retain their linkage after moving them around, and that
 a single player may bind themselves to multiple Obelisks. This also means you will need to relink yourself to an Obelisk if updating from
 v2.4.1 or prior.
 - To improve compatibility, Flasks of Power now use vanilla bucket behavior where possible

[Bugfixes]
- (1.21.1) Fixed Metamorpher name formatting recipes not dispensing results
- (1.21.1) Fixed Precision Dispeller not working on enchanted books
...
[Changes]
- Added support for experience items from other mods to the Fountain.
  - Items whitelisted in the config file may be converted into Cognitium by right-clicking a bound Fountain. Shift-right-clicking empties the entire stack.
  - Whitelisted items may be viewed in JEI, under the "Emptying" recipe category
- Memory Tablet area of effect is now determined by the absorption radius of the linked Obelisk
- Changed Memory Tablet recipe
...
- Added Memory Tablet
- Added Flint and Cognitive Alloy

[Changes]
- Metamorphers now have an additional setting to lock input slots
- Added many more transforming recipes
...
- Lowered cooldown and increased xp cost for Flask of Hades

[Bugfixes]
- (1.21.1) Fixed enlightened amulet not being placeable in curios slot
- (1.21.1) Fixed cognitive armor being stackable
- (1.21.1) Fixed obelisk options screen not displaying the correct background shader
- (1.21.1) Fixed obelisk buttons having the text "+x" instead of "-x" for drain functions
- (1.21.1) Fixed obelisk tooltips not showing up
- (1.21.1) Fixed dispeller showing "Insufficient XP" even when enough XP is present in the bound obelisk
- Fixed flasks of power being stackable
...
- Mending Neurogel may now be applied onto Anvils with either right-click or shift-right-click
- Optimized Metamorpher recipe loading and matching
- Merged name change recipes into a single recipe in JEI
