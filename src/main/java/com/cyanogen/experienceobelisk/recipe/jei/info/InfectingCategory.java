...
public class InfectingCategory implements IRecipeCategory<InfectingRecipe>{
    ...
    @Override
    public void draw(InfectingRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {

        Font font = Minecraft.getInstance().font;

        if(recipe.count > 0){
            counterArrow.draw(stack, 52, 8);
            font.draw(stack, "x"+recipe.count,67,8,0xFFFFFF);
        }

        arrow.draw(stack, 46, 22);
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, InfectingRecipe recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 99,19).setSlotName("output").addItemStack(recipe.getResultItem());
    }
